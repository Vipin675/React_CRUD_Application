...
const Register = () => {
  ...
  const addInputData = async(e) => {
    ...
    if (res.status === 422 || !data || !name || !email || !age || !phone || !work || !address || !descreption) {
    ...
    <>
      <div className='container register__container bg-dark'>
        ...
        <form className='text-white' action='/register' method="post">
          ...
          <button type="submit" onClick={addInputData} id='form_btn' className="btn btn-primary">Submit</button>
        </form>
      </div>
    </>
  )
